...
`bflat` | bflat outputs an executable when it finds a main() function, seemingly without the intermediate object files. This is no good, given I also need to link in the compiled boot loader. (nb. with a bit further effort, I suspect I might be able to include the bootloader asm directly in the csproj file, somehow)
